...
class Login extends CI_Controller
{
	...
	public function index()
	{
		if (!empty($this->session->userdata('Login_Auth'))) {
			if ($this->session->userdata('Login_Auth')['role'] == 'admin') {
	...
	public function AccountVerification()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$ArrayOTP = $this->Curd_model->Select('tbl_otp', ['sent' => $email]);
			$ArrayAccount = $this->Curd_model->Select('tbl_login', ['email' => $email]);

			if ($code == $ArrayOTP[0]['otp']) {
				$this->session->unset_userdata('account_verification');
				$adminArray['id'] =  $ArrayAccount[0]['id'];
				$adminArray['role'] =  $ArrayAccount[0]['role'];
				$this->session->set_userdata('Login_Auth', $adminArray);
				echo json_encode(array("statusCode" => 200, "msg" => 'Successfully Verify!', "url" =>  base_url()));
			} else {
				echo json_encode(array("statusCode" => 201, "msg" => 'Invalid OTP!'));
			}
		}
	...
	public function AccountSentOTP()
	{
		$email = $this->security->xss_clean($this->input->post('email'));
		$ArrayUserData = $this->Curd_model->Select('tbl_login', ['email' => $email]);
		if (empty($ArrayUserData)) {
			...
			$data = [
				'otp' => $OTP,
				'sent' => $ArrayUserData[0]['email'],
				'date_time' => date('Y-m-d h:i:s A'),
			];

			$masg = 'Hi,' . ucfirst($ArrayUserData[0]['name']) . '<br> We received a request for email verification. Enter the following verification code to account your active.';

			$this->Curd_model->Delete('tbl_otp', ['sent' => $ArrayUserData[0]['email']]);
			if (emialSent($OTP, $masg, $ArrayUserData[0]['email'], 'Account Verification OTP Resent') == true) {

				$this->Curd_model->insert('tbl_otp', $data);
				$this->session->set_userdata('account_verification', ['id' => $ArrayUserData[0]['id'], 'email' => $ArrayUserData[0]['email']]);
